...
from .models import Item1, Login, Hotmusic, User, Recmusic
...
def home(request, num='1'):
    ...
    music1 = Hotmusic.objects.all()[start_pos]
    music2 = Hotmusic.objects.all()[start_pos + 1]
    music3 = Hotmusic.objects.all()[start_pos + 2]
    music4 = Hotmusic.objects.all()[start_pos + 3]
    music5 = Hotmusic.objects.all()[start_pos + 4]
    ...
    lf = LoginForm()
    pic = ["img/post1.jpg"]
    Context = {'music_list': music_list, 'sf': sf, 'lf': lf, 'username': username, 'cur_page': curr_page,
               'has_previous': has_previous, 'has_next': has_next, 'previous_page': previous_page,
               'next_page': next_page, 'music1': music1, 'music2': music2, 'music3': music3, 'music4': music4,
               'music5': music5}
...
def search(request, num='1'):
    ...
    if 'q' in request.GET:
        ...
        if not q:
            ...
            lf = LoginForm()
            text = q
            Context = {'musics': music_list, 'query': q, 'all_page': all_page, 'count': count, 'cur_page': curr_page,
                       'has_previous': has_previous, 'has_next': has_next, 'sf': sf, 'lf': lf,
        ...
        sf = SignupForm()
        lf = LoginForm()
        Context = {'musics': music_list, 'query': text, 'all_page': all_page, 'count': count, 'cur_page': curr_page,
                   'has_previous': has_previous, 'has_next': has_next, 'sf': sf, 'lf': lf,
                   'previous_page': previous_page, 'next_page': next_page, 'username': username}
        return render(request, 'search_results.html', Context)
    return HttpResponse("Please submit a search term 50 characters or shorter.")
...
def signup(request):
    ...
    music1 = Hotmusic.objects.all()[0]
    music2 = Hotmusic.objects.all()[1]
    music3 = Hotmusic.objects.all()[2]
    music4 = Hotmusic.objects.all()[3]
    music5 = Hotmusic.objects.all()[4]
    # sf = SignupForm()
    # music_list = Item1.objects.all().order_by('item_id')
    lf = LoginForm()
    ...
    return render(request, 'home.html',
                  {'music_list': music_list, 'sf': sf, 'lf': lf, 'state': False, 'music1': music1, 'music2': music2,
                   'music3': music3, 'music4': music4, 'music5': music5})
    ...
    if request.method == "POST":
        ...
        if lf.is_valid():
            ...
            if user:
                ...
                Context = {'music_list': music_list, 'sf': sf, 'lf': lf, 'username': username, 'cur_page': curr_page,
                           'has_previous': has_previous, 'has_next': has_next, 'previous_page': previous_page,
                           'next_page': next_page, 'music1': music1, 'music2': music2, 'music3': music3,
                           'music4': music4, 'music5': music5}
                return render(request, 'home.html', Context)
            else:
